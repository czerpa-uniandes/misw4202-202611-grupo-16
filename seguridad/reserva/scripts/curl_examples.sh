...
curl -s "$BASE_URL/gateway/clientes/1" | jq .
...
curl -s "$BASE_URL/gateway/reservas" | jq .

echo "6) Ver logs de seguridad desde PostgreSQL"
docker compose exec -T postgres psql -U reserva -d reservadb -c "SELECT id,tipo_evento,cliente_id,motivo,fecha FROM seguridad_logs ORDER BY id DESC LIMIT 10;"
